<template>
  <q-card style="min-width: 300px">
    <form @submit.prevent="submitForm">
      ...
      <q-card-section>
        <q-input outlined  label="Name" placeholder="Name"  v-model="locationToSubmit.name" autofocus :rules="[val => !!val || 'Field is required']" ref="name"/>
      </q-card-section>
      <q-card-section>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions('settings', ['addLocation']),
			
			submitLocation() {
				this.addLocation(this.locationToSubmit)
				this.$emit('close')			
			},
			submitForm() {  
